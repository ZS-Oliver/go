...
public class MarketingManagerService extends BaseService<MarketingManagerVo, MarketingManagerQVo> {
    ...
    @Override
    protected void postUpdate(MarketingManagerVo newV, MarketingManagerVo oldV) throws ServiceException {
        if (!Strings.isNullOrEmpty(newV.getSchool())) {
            if (!Strings.isNullOrEmpty(newV.getSchool())) {
                employeeMapper.updateSchoolByMid(newV.getId(), newV.getSchool());
            }
        }
    }
