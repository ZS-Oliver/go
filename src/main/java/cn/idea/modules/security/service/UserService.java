...
public class UserService {
    ...
    public void login(UserVo uv, HttpSession session) throws ServiceException {
        try {
            ...
            String code = uv.getPhone();
            SecurityUtils.getSubject().login(new UsernamePasswordToken(uv.getPhone(), uv.getPwd()));

            // 登录成功，设置session信息
            if (UserUtil.isAdminCode(uv.getPhone())) {
    ...
    public void logout(HttpSession session) {
        log.info("用户退出登录：phone = {}", SecurityUtils.getSubject().getPrincipal());
    ...
    @Transactional
    public void resetMarketingManagerPwd(Integer id) throws ServiceException {
        MarketingManagerVo mv = marketingManagerMapper.find(id);
        if (mv == null) {
            throw new ServiceException("当前市场经理不存在");
        }
    ...
    @Transactional
    public void resetEmployeePwd(Integer id, Byte auth, Integer uid) throws ServiceException {
        EmployeeVo ev = employeeMapper.find(id);
        if (ev == null) {
            throw new ServiceException("当前员工不存在");
        }
        if (auth.equals(Role.MARKETING_MANAGER.getCode())) {
            ServiceException.when(!ev.getMid().equals(uid), "该员工不存在");
        }
    ...
    public void updateInfo(EmployeeVo employeeVo, Integer uid) throws ServiceException {
        ...
        MarketingManagerVo mv = marketingManagerMapper.findByPhone(employeeVo.getPhone());
        if (mv != null) {
            throw new ServiceException(String.format("手机号同市场经理%s重复", mv.getName()));
        }
        EmployeeVo evo = employeeMapper.findByPhone(employeeVo.getPhone());
        if (evo != null && !evo.getId().equals(uid)) {

            throw new ServiceException(String.format("手机号同员工%s重复", evo.getName()));
        }
    ...
    public void updateInfo(MarketingManagerVo marketingManagerVo, Integer uid) throws ServiceException {
        ...
        EmployeeVo ev = employeeMapper.findByPhone(marketingManagerVo.getPhone());
        if (ev != null) {
            throw new ServiceException(String.format("手机号同员工%s重复", ev.getName()));
        }
        MarketingManagerVo mv = marketingManagerMapper.findByPhone(marketingManagerVo.getPhone());
        if (mv != null && !mv.getId().equals(uid)) {
            throw new ServiceException(String.format("手机号同市场经理%s重复", mv.getName()));
        }
        ...
        ServiceException.when(res == 0, "修改错误，请重试");
        if (!Strings.isNullOrEmpty(marketingManagerVo.getSchool())) {
            employeeMapper.updateSchoolByMid(uid, marketingManagerVo.getSchool());
        }
    }
}
