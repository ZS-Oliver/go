...
//        assertEquals(carrierSession.getAttribute(SessionConst.AUTH), Role.CARRIER.getPhone());
//
...
//    }
    ...
    // 模拟员工登录成功
    @CanIgnoreReturnValue
    default MockHttpSession employeeLoginSuccess() {
    ...
    @CanIgnoreReturnValue
    default MockHttpSession marketingManagerLoginSuccess() {
        UserVo marketingManager = Users.genMarketingManager();
        MockHttpSession marketingSession = new MockHttpSession();

        try {
            mockMvc().perform(postJson(marketingSession, USER_TEMP_LOGIN_PATH, marketingManager))
                    .andExpect(isOKStatus());
        } catch (Exception e) {
            e.printStackTrace();
        }

        assertEquals(marketingSession.getAttribute(SessionConst.UID), 7);
        assertEquals(marketingSession.getAttribute(SessionConst.AUTH), Role.MARKETING_MANAGER.getCode());
